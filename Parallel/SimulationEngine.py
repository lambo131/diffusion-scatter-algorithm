...
class ScatterSimulator:
    def __init__(self, point_cloud: PLYManager, 
                 ...
                 p=1, num_balls=1000, 
                 max_spawn_points=500, 
                 render=False, spawn_radius_factor=1.0):
        ...
        self.t_max = min(5 * ball_radius, self.bbox_diagonal*0.02)
        # self.t_max = self.bbox_diagonal*0.02
        self.collision_margin = min(ball_radius * collision_margin_ratio, self.bbox_diagonal*0.002)
        # self.collision_margin = min(ball_radius * collision_margin_ratio, self.bbox_diagonal*0.05)
        self.p = p  # Likelyhood of spawning at new location
        self.total_balls = num_balls
        self.max_spawn_points = max_spawn_points
        self.render = render
        self.spawn_radius_factor = spawn_radius_factor
    ...
    def simulate_ball(self, max_steps=100, max_collisions=10, diffusion=False):
            ...
            if result == 'collision':
                ...
                if (collision_count > 2) and valid_2 and diffusion == True and generate_spawn == True:
                    ...
                    if last_collision_point is not None and self.data['dup_rate'][-1] > 0.5:
                        ...
                        k, idx, _ = self.point_cloud.kdtree.search_radius_vector_3d(spawn_point_candidate, self.ball_radius * self.spawn_radius_factor) # avoid intersecting with existing points
                        # k=0
                        if spawn_point_count > 0:
                            ...
                            k2, idx2, _ = spawn_point_kdtree.search_radius_vector_3d(spawn_point_candidate, self.ball_radius * self.spawn_radius_factor) # avoid too close to existing spawn points
                            k+=k2
                            # k=0
                        if k==0:
        ...
        if len(self.recent_dup_rates) == 10:
            ...
            threshold = 1.0  # e.g., 0.98 for max_steps=50
            if avg_recent_dup_rate > threshold:
                self.stop_flag = True
                print(f"*** STOP FLAG TRIGGERED ***")
